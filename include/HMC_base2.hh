...
class HMC_base {
  ...
  HMC_type* q; //!< class parameters definition to avoid many re-allocations in loops
  HMC_type* q0;
  HMC_type* p; //!< class momentum definition to avoid many re-allocations in loops
 public:
 HMC_base(int l, int nParams, int nOut_, int nRep_, int nBurn_) 
  ...
  inline HMC_type* it(HMC_type*); //!< one HMC iteration 
  void run();
  virtual HMC_type U(HMC_type*) = 0; //!< calculate U 
  virtual HMC_type* delU(HMC_type*) = 0; //!< calculate dU */
};
#endif
